...
状态：`0.1.0` Phase 1 基础 stdio 事件桥接、`TurnEventSink` 实时输出桥接、双向 request loop、真实 Turn Loop handler、RPC pending approval 等待队列、审批超时、pending run 取消语义、provider/tool 协作式取消信号和 Run Log 写入串行化已实现。
...
这意味着 Phase 1 已经具备真实 RPC 审批等待、取消、超时和协作式 provider/tool 停止语义，但还不是完全全双工的后台 streaming server：长时间 provider 请求仍会占用当前 `agent.sendTurn` request，事件也由 handler 返回给 request loop 后按顺序写出。后续需要加入独立事件 writer 队列和 reader/writer 并发编排，让 `agent.sendTurn` 可以更早返回 accepted，并在没有后续 request 的情况下持续推送事件。
...
- client 断连取消：当前取消由显式 `agent.cancel` 触发；后续 stdio reader/writer 分离后，应在 client 断连或前端 dispose 时主动取消 active run。
