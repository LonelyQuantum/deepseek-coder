...
状态：`0.1.0` Phase 1 基础 stdio 事件桥接、`TurnEventSink` 实时输出桥接、双向 request loop、真实 Turn Loop handler、RPC pending approval 等待队列、审批超时和 pending run 取消语义已实现。
...
- 事件发送队列：在异步 RPC Turn Loop handler 中保证同一 run 只有一个 writer 串行发送事件。
