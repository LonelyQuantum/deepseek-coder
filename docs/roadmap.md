...
状态：草案，Phase 1 Agent Core MVP、合并主线前离线最终验收、Phase 2 的 1M Context Capsule 核心收敛、Phase 2e 展示型 demo 扩展、Phase 3 VS Code 插件核心与共享 RPC 事件队列，以及 Phase 4 VS Code 深度集成均已完成。
...
Phase 3 已交付 VS Code 插件核心体验；Phase 4 已完成 VS Code 深度集成；TUI 进入 Phase 5，与生态扩展一起推进。Marketplace 发布不阻塞 Phase 4，当前已具备可安装 VSIX alpha / pre-release 产物和安装说明。Phase 2e 展示型 demo 已经给 VS Code Context Viz / Approval / Run Log UI 提供可观察样本。
...
- Terminal command approval 已展示命令、cwd、风险等级、风险原因、输出摘要字段和持久化选项。
