...
`crates/agent-rpc` 已实现 Run Log 事件到 `agent.event` notification 的基础桥接；后续 request loop 会继续把工具请求、审批请求和工具结果暴露给 CLI/TUI/VS Code。
...
当前执行层已接入基础 Agent Turn Loop、审批策略和 run log。写入与命令执行会触发审批请求；默认审批策略拒绝执行，测试可使用显式 auto-approve 策略验证已批准路径。Run Log 事件已能通过基础 RPC 桥接发送给前端；真实用户审批等待和网络/破坏性风险升级仍需要在 RPC/CLI 接入后实现。
...
- 在 `crates/agent-rpc` request loop 中接入工具请求、审批请求、工具结果和 patch 事件的实时发送与重放。
