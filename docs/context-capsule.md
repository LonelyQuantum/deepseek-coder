...
状态：草案，Phase 1 基础 Context Builder 已实现；Phase 2a/2b/2c 已完成，下一步进入 Phase 2d 的大上下文验收和截断/体积边界。
...
| Phase 2c | 已接入 `agent.sendTurn.attachments` 的 file/selection/explicit_content/diagnostic；新增 `provider.completed` usage/cache/stream 摘要事件。 | 已覆盖 attachment 越界、重复、超大小限制、RPC file attachment 和 provider summary 事件测试；更大 cache hit/miss 手动样本留到 Phase 2d 前增强。 |
...
- 扩展 DeepSeek cache hit/miss 手动实验样本，在大上下文重复前缀场景下记录更清晰的 ignored live 验收过程。
