...
状态：草案，Phase 1 基础 Context Builder 已实现；Phase 2a-1 的 `read_file` 摘要元数据和 Phase 2a-2 的结构化 Context Capsule renderer 已完成，下一步进入 workspace manifest v0。
...
| 阶段 | 内容 | 默认验收 |
| --- | --- | --- |
| Phase 2a | 已完成 `read_file` 增加 `sha256` / `sizeBytes`、`ContextCapsule` / `ContextSection` / `CachePlacement` 和稳定 renderer；继续实现 workspace manifest v0；接入 manifest summary 与 `context.built` payload。 | 离线 fixture 仓库 manifest、hash、ignore、truncation 和渲染稳定性测试。 |
...
- 基于已实现的 `CachePlacement::{StablePrefix, DynamicPrelude, TurnSuffix}` 布局，接入 provider cache usage 字段，并用 DeepSeek cache hit/miss 做 ignored live 验证。
