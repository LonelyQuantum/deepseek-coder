...
状态：草案，Phase 1 基础 Context Builder 已实现；Phase 2a 已完成，下一步进入 Phase 2b 的 TokenEstimator 与稳定前缀缓存验证。
...
- token 报告：输出 `inputTokens`、`maxInputTokens`、stable/dynamic/suffix section token、included sources、omitted sources、manifest hash / truncation 和估算器说明。
