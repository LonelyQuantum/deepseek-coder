...
状态：Phase 1 基础编排、TurnProvider async / streaming 边界、真实 DeepSeek 文本 streaming 联网验收、streaming tool call 增量拼装验证、基础 RPC 事件桥接、双向 request loop、真实 RPC Turn Loop handler、`TurnEventSink` 实时事件输出、CLI 交互式审批、RPC pending approval 等待队列、审批超时、取消语义、provider/tool 协作式取消信号、Run Log 写入串行化和 Run summary metadata 已实现；TUI/VS Code 已有审批交互原语，尚未接入真实 RPC 队列。
...
- 全双工异步 RPC run 执行和独立事件发送队列；当前 `AgentTurnLoopRpcHandler` 已有后台 worker、审批等待和协作式取消，但事件仍在 request 返回时由 request loop flush。
...
- 扩展取消模型到 client 断连、进程树清理和多 active run 场景。
